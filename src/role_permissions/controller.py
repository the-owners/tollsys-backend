...
from src.role_permissions import service
...
def assign_permission_to_role(
    role_id: int, permission_id: int, current_user: CurrentUser, session: SessionDep
):
...
def get_permissions_by_role(
    role_id: int, current_user: CurrentUser, session: SessionDep
):
